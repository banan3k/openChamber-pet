...
export function createPetServer({ pets, defaultPetId, onSelect }) {
  ...
  function handler(req, res) {
    ...
    if (url.pathname === "/state" && req.method === "GET") {
      return json(res, 200, { state: currentState, petId: currentPetId, tasks: currentTasks, toggleSeq })
    }
    ...
    if (url.pathname === "/events") {
      ...
      res.write(`data: ${JSON.stringify({ state: currentState, petId: currentPetId, tasks: currentTasks })}\n\n`)
      clients.add(res)
